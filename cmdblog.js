...
(function() {
  ...
  root.cmdBlog._putStr = function(str) {
    if (str === "\n") {
      $(".command").append($("<div class='grid'></div>"));
    }
    return this.html(this.html() + str);
  };

  root.cmdBlog._displayNext = function(objSelector, displayId) {
    var cacheStr, cntDisplayed, displayed, nowDisplayId, str, timer, timerCmd;
    ...
    if (nowDisplayId === displayId && cntDisplayed < str.length) {
      if (str[cntDisplayed] === '{') {
        ...
        while (str[cntDisplayed] !== '}') {
          cacheStr += str[cntDisplayed];
          displayed += str[cntDisplayed++];
        }
        this.putStr(cacheStr);
      } else {
        this.putStr(str[cntDisplayed]);
        displayed += str[cntDisplayed];
      }
  ...
  root.cmdBlog.displayResult = function(str) {
    return $(".result").display(".result", str);
  };
  ...
  root.cmdBlog.processCommand = function(command) {
    ...
    if (tokens.length === 0) {
      return root.cmdBlog.displayResult("");
    } else if (root.cmdBlog.commandFunctionList[tokens[0]]) {
  ...
  $(function() {
    ...
    jQuery.fn.putStr = root.cmdBlog._putStr;
    $(".cmdprompt").html(root.cmdBlog.promptStr);
    $(".cmdinput").focus();
